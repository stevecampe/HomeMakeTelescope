...
module mirror_to_base_fixation()
{
    difference()
    {
        cylinder(h=1, r=166/2);
        translate([0,0,-1]) cylinder(h=1+2, r=156/2);
    }
    difference()
    {
        cylinder(h=35, r=165.5/2);
        translate([0,0,-1]) cylinder(h=35+2, r=160.5/2);
    ...
    difference()
    {
        translate([0,0,hoogte_mirror_base])
            rotate([0,180,0])
                difference()
                {
                    cylinder(h=10, r1 = buiten_diameter / 2 + 5,r2= buiten_diameter / 2);
...
module alu_bar_holder()
{
    difference()
    {
        ...
        //schroefgaatje
        translate([0,0,0]) rotate([90,0,0])  cylinder(h=alu_holder_size+2*alu_holder_thickness + 2, r=2,center=true);
        translate([0,0,-hoogte_mirror_base/2])cylinder(h=10, r=2,center=true);
    }
}
module alu_bar_hol(hoogte)
{
    difference()
    {
        ...
        //schroefgaatje
        translate([0,0,hoogte/3]) rotate([90,0,0])  cylinder(h=alu_holder_size+2*alu_holder_thickness + 2, r=2,center=true);
        translate([0,0,0]) rotate([90,0,0])  cylinder(h=alu_holder_size+2*alu_holder_thickness + 2, r=2,center=true);
        translate([0,0,-hoogte/3]) rotate([90,0,0])  cylinder(h=alu_holder_size+2*alu_holder_thickness + 2, r=2,center=true);
        translate([0,0,-hoogte/2]) cylinder(h=10, r=2,center=true);
    }
}

hoogte_sec_mirror_base = 35;
sec_base_height = 5;
module Small_mirror_base()
{
    ...
    difference()
    {
        ...
        rotate([0,0,45]) translate([buiten_diameter/2 + alu_holder_size/2 + 2,0,hoogte_sec_mirror_base/2]) cube([alu_holder_size,alu_holder_size,100],center=true);
        rotate([0,0,45+90]) translate([buiten_diameter/2 + alu_holder_size/2 + 2,0,hoogte_sec_mirror_base/2]) cube([alu_holder_size,alu_holder_size,100],center=true);
        rotate([0,0,45+180]) translate([buiten_diameter/2 + alu_holder_size/2 + 2,0,hoogte_sec_mirror_base/2]) cube([alu_holder_size,alu_holder_size,100],center=true);
        rotate([0,0,45-90]) translate([buiten_diameter/2 + alu_holder_size/2 + 2,0,hoogte_sec_mirror_base/2]) cube([alu_holder_size,alu_holder_size,100],center=true);
    }
}
...
module secondary_mirror()
{
    difference()
    {
        ...
        rotate([0,0, 60]) translate([10,-3.2,0]) cube([10,6.4,8]);
        rotate([0,0,180]) translate([10,-3.2,0]) cube([10,6.4,8]);
        rotate([0,0,-60]) translate([10,-3.2,0]) cube([10,6.4,8]);
    ...
    */   
    ...
    difference()
    {
        ...
        {
            cylinder(h=90, r=2.5);
            rotate([0,0,   0]) translate([0,-1,0]) cube([35/2,2,90 ],center=false);
            rotate([0,0, 120]) translate([0,-1,0]) cube([35/2,2,90 ],center=false);
            rotate([0,0, 240]) translate([0,-1,0]) cube([35/2,2,90 ],center=false);

        }
        translate([-20,0,100]) rotate([0,45,0]) cube(100, center=true);
    }        
    ...
    translate([0,0,-hoogte_eye_piece/2])
    {
        difference()
        {
            ...
            difference()
            {
                ...
                translate([0,0,40]) rotate([0,90,0]) cylinder(h=250,r=31,center=true);
            } 
...
//vijsgat();
//translate([0,0,-30]) rotate ([180,0,0]) mirror_plate();
//mirror_base();
...
//translate([0,0,-41]) eye_piece_connector();
